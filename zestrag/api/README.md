# ZestRAG Server and WebUI

The ZestRAG Server is designed to provide a Web UI and API support. The Web UI facilitates document indexing, knowledge graph exploration, and a simple RAG query interface. ZestRAG Server also provides an Ollama-compatible interface, aiming to emulate ZestRAG as an Ollama chat model. This allows AI chat bots, such as Open WebUI, to access ZestRAG easily.
...
### Before Starting ZestRAG Server

ZestRAG necessitates the integration of both an LLM (Large Language Model) and an Embedding Model to effectively execute document indexing and querying operations. Prior to the initial deployment of the ZestRAG server, it is essential to configure the settings for both the LLM and the Embedding Model. ZestRAG supports binding to various LLM/Embedding backends:
...
It is recommended to use environment variables to configure the ZestRAG Server. There is an example environment variable file named `env.example` in the root directory of the project. Please copy this file to the startup directory and rename it to `.env`. After that, you can modify the parameters related to the LLM and Embedding models in the `.env` file. It is important to note that the ZestRAG Server will load the environment variables from `.env` into the system environment variables each time it starts. **ZestRAG Server will prioritize the settings in the system environment variables to .env file**.
...
### Starting ZestRAG Server

The ZestRAG Server supports two operational modes:
...
```
zestrag-gunicorn --workers 4
```
The `.env` file **must be placed in the startup directory**.

Upon launching, the ZestRAG Server will create a documents directory (default is `./inputs`) and a data directory (default is `./rag_storage`). This allows you to initiate multiple instances of ZestRAG Server from different directories, with each instance configured to listen on a distinct network port.
...
> - The requirement for the .env file to be in the startup directory is intentionally designed this way. The purpose is to support users in launching multiple ZestRAG instances simultaneously, allowing different .env files for different instances.
> - **After changing the .env file, you need to open a new terminal to make  the new settings take effect.** This because the ZestRAG Server will load the environment variables from .env into the system environment variables each time it starts, and ZestRAG Server will prioritize the settings in the system environment variables.

### Launching ZestRAG Server with Docker
...
* Start the ZestRAG Server using the following commands:
```shell
docker compose up
...
### Deploying ZestRAG Server with docker without cloneing the repository

* Create a working folder for ZestRAG Server:
...
The ZestRAG Server can operate in the `Gunicorn + Uvicorn` preload mode. Gunicorn's multiple worker (multiprocess) capability prevents document indexing tasks from blocking RAG queries. Using CPU-exhaustive document extraction tools, such as docling, can lead to the entire system being blocked in pure Uvicorn mode.

Though ZestRAG Server uses one worker to process the document indexing pipeline, with the async task support of Uvicorn, multiple files can be processed in parallel. The bottleneck of document indexing speed mainly lies with the LLM. If your LLM supports high concurrency, you can accelerate document indexing by increasing the concurrency level of the LLM. Below are several environment variables related to concurrent processing, along with their default values:
...
### Install ZestRAG as a Linux Service

Create your service file `zestrag.service` from the sample file: `zestrag.service.example`. Modify the `WorkingDirectory` and `ExecStart` in the service file:

```text
Description=ZestRAG Ollama Service
WorkingDirectory=<zestrag installed directory>
ExecStart=<zestrag installed directory>/zestrag/api/zestrag-api
```

Modify your service startup script: `zestrag-api`. Change your Python virtual environment activation command as needed:

```shell
#!/bin/bash

# your python virtual environment activation
source /home/netman/zestrag-xyj/venv/bin/activate
# start zestrag api server
zestrag-server
```

Install ZestRAG service. If your system is Ubuntu, the following commands will work:

```shell
sudo cp zestrag.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl start zestrag.service
sudo systemctl status zestrag.service
sudo systemctl enable zestrag.service
```

## Ollama Emulation

We provide Ollama-compatible interfaces for ZestRAG, aiming to emulate ZestRAG as an Ollama chat model. This allows AI chat frontends supporting Ollama, such as Open WebUI, to access ZestRAG easily.

### Connect Open WebUI to ZestRAG

After starting the zestrag-server, you can add an Ollama-type connection in the Open WebUI admin panel. And then a model named `zestrag:latest` will appear in Open WebUI's model management interface. Users can then send queries to ZestRAG through the chat interface. You should install ZestRAG as a service for this use case.
...
The default query mode is `hybrid` if you send a message (query) from the Ollama interface of ZestRAG. You can select query mode by sending a message with a query prefix.

A query prefix in the query string can determine which ZestRAG query mode is used to generate the response for the query. The supported prefixes include:
...
For example, the chat message `/mix What's ZestRAG?` will trigger a mix mode query for ZestRAG. A chat message without a query prefix will trigger a hybrid mode query by default.

`/bypass` is not a ZestRAG query mode; it will tell the API Server to pass the query directly to the underlying LLM, including the chat history. So the user can use the LLM to answer questions based on the chat history. If you are using Open WebUI as a front end, you can just switch the model to a normal LLM instead of using the `/bypass` prefix.

`/context` is also not a ZestRAG query mode; it will tell ZestRAG to return only the context information prepared for the LLM. You can check the context if it's what you want, or process the context by yourself.

### Add user prompt in chat

When using ZestRAG for content queries, avoid combining the search process with unrelated output processing, as this significantly impacts query effectiveness. User prompt is specifically designed to address this issue — it does not participate in the RAG retrieval phase, but rather guides the LLM on how to process the retrieved results after the query is completed. We can append square brackets to the query prefix to provide the LLM with the user prompt:
...
By default, the ZestRAG Server can be accessed without any authentication. We can configure the server with an API Key or account credentials to secure it.
...
ZestRAG API Server implements JWT-based authentication using the HS256 algorithm. To enable secure access control, the following environment variables are required:
...
RESOURCE_GROUP_NAME=ZestRAG
LOCATION=swedencentral
RESOURCE_NAME=ZestRAG-OpenAI
...
## ZestRAG Server Configuration in Detail
...
ZestRAG supports binding to various LLM/Embedding backends:
...
ZestRAG uses 4 types of storage for different purposes:
...
You cannot change storage implementation selection after adding documents to ZestRAG. Data migration from one storage implementation to another is not supported yet. For further information, please read the sample env file or config.ini file.

### ZestRAG API Server Command Line Options
...
| --key                 | None          | API key for authentication. Protects the ZestRAG server against unauthorized access                                            |
...
### For JWT Auth
# AUTH_ACCOUNTS='admin:admin123,user1:pass456'
# TOKEN_SECRET=your-key-for-ZestRAG-API-Server-xxx
...
Handle chat completion requests. Routes user queries through ZestRAG by selecting query mode based on query prefix. Detects and forwards OpenWebUI session-related requests (for metadata generation task) directly to the underlying LLM.

```shell
curl -N -X POST http://localhost:9621/api/chat -H "Content-Type: application/json" -d \
  '{"model":"zestrag:latest","messages":[{"role":"user","content":"猪八戒是谁"}],"stream":true}'
```
...
Handle generate completion requests. For compatibility purposes, the request is not processed by ZestRAG, and will be handled by the underlying LLM model.
